...
from runtime.audit_log import stable_hash, update_audit_run
from execution.order_planner import PLAN_PATH
...
def place_payload(
    ...
    *,
    ...
    if require_confirm and str(confirmation.get("plan_key", "")).strip() != pk:
        raise RuntimeError(
            "Plan is not confirmed. Run `python -m execution.groww_order_runner --mode confirm` first."
        )
